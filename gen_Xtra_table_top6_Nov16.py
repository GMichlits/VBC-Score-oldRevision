...
#species_list = ['mm10', 'dm6']
#species_list = ['mm10', 'ce11','rn6', 'dm6', 'xenTro9']
...
penalty_noncommon_CDS_isoforms = 0.1
...
for Species in species_list:
    print(Species)
    print('open Species data: ' + str(Species))
    basic_property_filename = Species + '_cut.propertie.new2.txt'
    Basic_property_file = open('../../VBC_cross_species/' + basic_property_filename, 'r')
    gtf_file = open('../../VBC_cross_species/' + Species + '_refGene.gtf','r')
    OFFtarget_filename = Species + '_offtarget.txt'
    OFFtarget_file = open('../../VBC_cross_species/' + OFFtarget_filename)

    outfile_top6 = open('top_6/' + Species + '_top' + str(topX_sgRNA_autopickmode) + '_sgRNAs.txt','w')
    outfile_all = open('all_sgRNAs/' + Species + '_all_sgRNAsthrash.txt','w')

    print('open sgRNA properties')
    if Species == 'hg38':
        Species = 'Hm'
    print('load genenames')
    ...
    for i,line in enumerate(Basic_property_file):
        if i > 0:
            ...
            #if pos not in Gene_d:
            #    Gene_d[pos] = gene_name
    ...
    #pickle.dump(Gene_d,open('../../Gen_properties/' + Species + '/property_sav/Gene_d_Nov18.sav','wb'))
    ...
    print('read in exon assignements gtf file')

    exon_dict = {}
    # remember isoforms_transcripts per geneID and later use this to check of pos targets seq common to all transcripts or not
    isoform_dict = {}
    #isoform_dict[chr][geneID or genename][transcriptID] = [[start,stop],[start,stop],[start,stop]]
    ...
    for line in gtf_file:
        ...
        if col[2] == 'exon':
            ...
            gene_ID = col[8].split(';')[0].replace('gene_id ','').replace('"', '').replace(' ','')
            transcript_ID = col[8].split(';')[1].replace('transcript_id ','').replace('"', '').replace(' ','')
            gene_name = col[8].split(';')[4].replace('gene_name ','').replace('"', '').replace(' ','')
            if not chr in exon_dict:
                exon_dict[chr] = {}
            if not chr in isoform_dict:
                isoform_dict[chr] = {}
            ...
            if gene_ID == gene_name:
                ...
                if not gene_ID in isoform_dict[chr]:
                    isoform_dict[chr][gene_ID] = {}
                if not transcript_ID in isoform_dict[chr][gene_ID]:
                    isoform_dict[chr][gene_ID][transcript_ID] = []
                isoform_dict[chr][gene_ID][transcript_ID].append([start,stop])
            else:
                exon_dict[chr][gene_ID].append([start,stop,exon])
                ...
                if not transcript_ID in isoform_dict[chr][gene_ID]:
                    isoform_dict[chr][gene_ID][transcript_ID] = []
                isoform_dict[chr][gene_ID][transcript_ID].append([start,stop])
                if not gene_name in isoform_dict[chr]:
                    isoform_dict[chr][gene_name] = {}
                if not transcript_ID in isoform_dict[chr][gene_name]:
                    isoform_dict[chr][gene_name][transcript_ID] = []
                isoform_dict[chr][gene_name][transcript_ID].append([start,stop])
    ...
    for pos in nt30_d:
        ...
        if not 'Un' in pos.split(':')[0] and not 'N' in nt30_d[pos] and not '' == nt30_d[pos]:
            ...
            # penality for copy number (OT90 is an estimate of likely off-target sites in the coding genome):
            if 1 <= OT95:
                VBC_score_picking = VBC_score_picking * penalty_OT95over0
            if 1 <= OT90 < 3:
            ...
            #if common_CDS == 0:
            #    VBC_score_picking = VBC_score_picking * 0.05
            if naturalG_19 == 1:
            ...
            genename = Gene_d[pos]
            #determine if pos is isoform specific or not.
            cutsite = int(pos_to_cutsite(pos))
            chr = chromosome_name
            isoform_presence_list = []
            if genename in isoform_dict[chr]:
                for transcript in isoform_dict[chr][genename]:
                    presence = 0
                    for exoncoord in isoform_dict[chr][genename][transcript]:
                        start_ex = int(exoncoord[0])
                        stop_ex = int(exoncoord[1])
                        if start_ex <= cutsite <= stop_ex:
                            presence = 1
                            break
                    isoform_presence_list.append(presence)
            #print(genename + '\t' + pos + '\t' + str(isoform_presence_list))
            # a 0 in the isoform presence list means that a trasncript for that gene was found were the sgRNA does not cut
            # the score for automated picking will be penalized.
            if 0 in isoform_presence_list:
                VBC_score_picking = VBC_score_picking * penalty_noncommon_CDS_isoforms
    ...
    outfile_top6.write('gene' + '\t' + 'sgRNA' + '\t' + 'Position' + '\t' + 'Exon number refGene' + '\t' + 'Auto-pick "top 6"' + '\t' + 'VBC score' + '\t'
    ...
    for gene in sorted(sgRNAs):
        ...
        for i in range(topX_sgRNA_autopickmode):
            for item in sorted(sgRNAs[gene].items(), key = lambda t: t[1], reverse=True):
                ...
                orientation_picked = pos.split('(')[1][0]
                ...
                for test_set in exon_dict[chr][gene]:
                    ...
                    if test_start < start < test_stop or test_start < stop < test_stop:
                        exon = test_exon
                        break #breaks the loop for test in exon... because exon was already foung
                ...
                if not pos in pep_d:
                    pep_d[pos] = 'NA'
                # printout of genes processed in alphabetic order to know how long the script is running olny print out first guide per gene
                if not gene == oldgene:
                ...
                for pos in sgRNAs[gene]:
                    ...
                    candiate_orientation = pos.split('(')[1][0]
                    if orientation_picked == candiate_orientation and abs(cutsite_picked_sgRNA - candidate_cutsite) <= 1:
                        VBC_score_picking = sgRNAs[gene][pos]
                        VBC_score_picking = VBC_score_picking * proximity_lessthanonebp_penality
                        sgRNAs[gene][pos] = VBC_score_picking
                # break after 1st guide and rerun until 6 sgRNAs picked (outside loop).
                break
            #print(hitzone)
    #pickle.dump(top6_d, open(Species + '_top6_d.sav','wb'))
